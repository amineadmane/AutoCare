...
def pourcentage_vehicules_per_marque(request):
    ...
    stats_modele = Vehicule.objects.values('modele__marque__id').annotate(
        modele_nom=F('modele__marque__nom'), nb=Count('*'))
...
def pourcentage_vehicules_per_modele(request, marque):
    ...
    stats_modele = Vehicule.objects.filter(modele__marque=marque).values(
        'modele__id').annotate(modele_nom=F('modele__nom'), nb=Count('*'))
    for modele in stats_modele:
        modele['pourcentage'] = modele['nb'] * 100 / total_modele
